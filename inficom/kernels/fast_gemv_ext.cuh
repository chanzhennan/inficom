...
__global__ __forceinline__ void fast_gemv_acc_fp16_with_llama2_rope_kernel(
    half* WQ, half* WK, half* WV, half* X, float* freq, half* Q, half* K,
    half* V, int len, int dim, int hn, int hs, unsigned int num_per_thread) {
  // each thread load num_per_thread elements from global
  unsigned int tid = threadIdx.x;
  unsigned int row = (blockIdx.y * blockDim.y + threadIdx.y) << 1;
  half* weight_ptr = (row < dim) ? &WQ[0] : (row < 2 * dim) ? &WK[0] : &WV[0];
...
#pragma unroll
  for (int iter = 0; iter < DIV_UP(num_per_thread, 8); iter++) {
    ...
    *(float4*)(&mat_val[4]) =
        *(float4*)(weight_ptr + ((row + 1) % dim) * dim + j);
...
__global__ __forceinline__ void fast_gemv_acc_fp16_with_chatglm2_rope_kernel(
    half* WQKV, half* BQKV, half* X, half* freq, half* Q, half* K, half* V,
    int len, int dim, int hn, int gn, int hs, unsigned int num_per_thread) {
...
#pragma unroll
  ...
  if (row < dim) {
    ...
    if (idx < hs / 2) {
      ...
      update_sum.x = to_rotate.x * gsum.x - to_rotate.y * gsum.y;
      update_sum.y = to_rotate.x * gsum.y + to_rotate.y * gsum.x;
      *(half2*)(&K[len * (gn * hs) + (row % dim)]) =
          __float22half2_rn(update_sum);
    } else {
      *(half2*)(&K[len * (gn * hs) + (row % dim)]) = __float22half2_rn(gsum);
    }
  } else if (row < (dim + 2 * gn * hs)) {
    *(half2*)(&V[len * (gn * hs) + (row % dim % (gn * hs))]) =
        __float22half2_rn(gsum);
  }
}
...
__global__ __forceinline__ void fast_gemv_acc_fp16_with_bias_kernel(
    half* mat, half* b, half* vec, half* res, unsigned int n,
...
/*
    GEMV kernel using FP16 to accumulate. For QKV Proj with separate weights and
   bias.
*/
__global__ __forceinline__ void fast_gemv_acc_fp16_for_qkv_proj_kernel(
    ...
    half* Q, half* K, half* V, int len, int dim, int hn, int hs,
    unsigned int num_per_thread) {
  // each thread load num_per_thread elements from global
  unsigned int tid = threadIdx.x;
  unsigned int row = (blockIdx.y * blockDim.y + threadIdx.y) << 1;
  half* weight_ptr = (row < dim) ? &WQ[0] : (row < 2 * dim) ? &WK[0] : &WV[0];
  half* bias_ptr = (row < dim) ? &BQ[0] : (row < 2 * dim) ? &BK[0] : &BV[0];
  unsigned int start_idx = threadIdx.x;
  half2 vec_val[4];
  half2 mat_val[8];
...
#pragma unroll
  ...
  if (row < dim) {
    ...
    *(half2*)(&K[len * dim + (row % dim)]) =
        __hadd2(__float22half2_rn(gsum), bias);
  } else if (row < 3 * dim) {
    *(half2*)(&V[len * dim + (row % dim)]) =
        __hadd2(__float22half2_rn(gsum), bias);
  }
}

__global__ __forceinline__ void fast_gemv_acc_fp16_residual_kernel(
    half* mat, half* vec, half* r, half* res, unsigned int n,
    unsigned int num_per_thread) {
  // each thread load num_per_thread elements from global
  unsigned int tid = threadIdx.x;
  unsigned int row = (blockIdx.y * blockDim.y + threadIdx.y) << 1;
...
__global__ __forceinline__ void fast_gemv_acc_fp16_bias_relu_kernel(
    half* x, half* w1, half* b1, int bs, int dim, int h_dim, half* res) {
  // each thread load num_per_thread elements from global
  unsigned int tid = threadIdx.x;
...
__global__ __forceinline__ void fast_gemv_acc_fp16_bias_residual_kernel(
    half* mat, half* b, half* vec, half* r, half* res, unsigned int n,
    unsigned int num_per_thread) {
  // each thread load num_per_thread elements from global
  unsigned int tid = threadIdx.x;
  unsigned int row = (blockIdx.y * blockDim.y + threadIdx.y) << 1;
